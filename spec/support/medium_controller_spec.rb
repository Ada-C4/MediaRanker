RSpec.shared_examples "a medium controller" do
  ...
  describe "POST 'create'" do
    ...
    it "unsuccessful create does not add to the database" do
      media_count = model.all.count
      post :create, bad_params
      expect(model.all.count).to eq media_count
    end

    it "unsuccessful create renders new template" do
      post :create, bad_params
      expect(response.status).to eq 200
      expect(subject).to render_template :new
    end
  ...
  describe "DELETE 'destroy'" do
    ...
    it "removes the media from the database" do
      delete :destroy, id: medium.id
      expect(model.all).not_to include(medium)
    end
  end
